...
SEARCH_LIMIT= 20
DESIRED_FIELDS= ['rating', 'review_count', 'name', 'url', 'is_closed', 'snippet_text']
...
def write_output(businesses, term, location):
    ...
    outfile_name= '{0}_{1}_{2}.csv'.format(term.replace(' ', '-').replace(',', '-'), \
        location.replace(' ', '-').replace(',', '-'), ts)
    o= open(outfile_name, 'a+')
    csv_writer= csv.writer(o)

    ### TAKE VAR NAMES FROM INTERSECTION OF DESIRED_FIELDS
    ###   AND FIELDS AVAILABLE IN FIRST RETURNED BUSINESS
    csv_writer.writerow([field.encode('ascii', 'ignore') for field in businesses[0].keys() \
